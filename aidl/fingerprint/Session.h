...
public:
    ...
    Session(fingerprint_device_t* fp_device, int userId, std::shared_ptr<ISessionCallback> cb);

    ndk::ScopedAStatus generateChallenge() override;
    ndk::ScopedAStatus revokeChallenge(int64_t challenge) override;
    ndk::ScopedAStatus enroll(const HardwareAuthToken& hat, ICancellationSignal* _aidl_return) override;
    ndk::ScopedAStatus authenticate(int64_t operationId, ICancellationSignal* _aidl_return) override;
    ndk::ScopedAStatus detectInteraction(ICancellationSignal* _aidl_return) override;
    ndk::ScopedAStatus enumerateEnrollments() override;
    ndk::ScopedAStatus removeEnrollments(std::vector<int32_t> enrollmentIds) override;
    ...
    ndk::ScopedAStatus enrollWithContext(const HardwareAuthToken& hat, const OperationContext& context, ICancellationSignal* _aidl_return) override;
...
private:
    ...
    static AcquiredInfo VendorAcquiredFilter(int32_t error, int32_t* vendorCode);
    ...
    std::shared_ptr<ISessionCallback> mSessionCallback;
    std::mutex mSessionCallbackMutex;
